...
---------------
FIXED - the CP balance sticking at 999,999,999. `Client::giveCP` treated a reward of 0 as an overflow and set the balance to `_I64_MAX`; `Rival::LoseCP` returns 0 for any battle shorter than a kilometre, so a single short loss broke the account for good (the client shows anything from 1,000,000,000 up as the 999,999,999 cap, and the battle result screen adds the reward to the low 32 bits, which is where "win 35CP, total 34CP" came from). `giveCP` now ignores non-positive rewards and clamps to `CP_LIMIT` (999,999,999), and `setCP` clamps on load so accounts already broken in the database come back into a displayable range. Reset a wrecked balance with `!set cp <amount>` in game, or `UPDATE account_data SET cp = 0 WHERE license = <id>;`.
---------------
